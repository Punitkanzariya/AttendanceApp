...
export default function LeaveBalanceBoxes({ userLeaveBalance, isLoading, leaves = [], leaveTypes = [], user }: LeaveBalanceBoxesProps) {
  ...
  leaveCategories.forEach(category => {
    const leaveType = leaveTypes.find(lt => lt.name.toLowerCase().includes(category.key));
    const max = userLeaveBalance?.[category.key] !== undefined 
      ? userLeaveBalance[category.key] 
      : (user?.leaveBalances?.[leaveType?.leaveTypeId || category.key] !== undefined 
  ...
  return (
    <View style={styles.leaveGrid}>
      {categoriesWithTotal.reduce((resultArray, item, index) => { 
        ...
        return resultArray;
      }, [] as typeof categoriesWithTotal[]).map((row, rowIndex) => (
        <View key={rowIndex} style={styles.leaveRow}>
          {row.map((category) => {
            ...
            if (category.key === 'total') {
              ...
              max = userLeaveBalance?.[category.key] !== undefined 
                ? userLeaveBalance[category.key] 
                : (user?.leaveBalances?.[leaveType?.leaveTypeId || category.key] !== undefined 
                  ? user.leaveBalances[leaveType?.leaveTypeId || category.key] 
                  : (leaveType?.annualQuota || 0));
            ...
            return (
              <View key={category.key} style={styles.leaveCard}>
                <View style={styles.cardHeader}>
                  <View style={[styles.leaveIconRing, { borderColor: category.color }]}>
                    <Ionicons name="briefcase-outline" size={14} color={Colors.text.primary} />
                  </View>
                ...
                <Text style={styles.leaveTakenTxt}>{isLoading ? "-" : taken} Taken • {isLoading ? "-" : max} Total</Text>
              </View>
            );
          })}
