...
export default function FinanceExpenseScreen() {
  const { user } = useAuthStore();
  const [expenses, setExpenses] = useState<ExpenseRequest[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [filter, setFilter] = useState<string>('pending_finance');
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAllExpenses(
      ['pending_finance', 'reimbursed', 'rejected'],
      (data) => {
        setExpenses(data);
        setIsLoading(false);
      }
  ...
  const filteredExpenses = useMemo(() => {
    ...
    if (filter === 'approved') return expenses.filter(e => e.status === 'reimbursed');
  ...
  const renderItem = ({ item }: { item: ExpenseRequest }) => (
    <TouchableOpacity 
      style={styles.card} 
      activeOpacity={0.7}
      onPress={() => item.status === 'pending_finance' ? openReviewModal(item) : null}
    >
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <View style={styles.filterRow}>
        {(['pending_finance', 'approved', 'rejected', 'all'] as const).map(f => (
          <TouchableOpacity 
            key={f}
          ...
          >
            <Text style={[styles.filterTxt, filter === f && styles.filterTxtActive]}>
              {f === 'pending_finance' ? 'Pending' : f === 'approved' ? 'Reimbursed' : f.charAt(0).toUpperCase() + f.slice(1)}
            </Text>
            {f === 'pending_finance' && expenses.filter(e => e.status === 'pending_finance').length > 0 && (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Process Reimbursement</Text>
              ...
              <>
                <View style={styles.modalScroll}>
                  ...
                  {selectedExpense.attachmentUrl && (
                    <TouchableOpacity style={styles.viewBillBtn} onPress={() => openAttachment(selectedExpense.attachmentUrl!)}>
                      ...
                      <Text style={styles.viewBillText}>Verify Attached Bill</Text>
                    </TouchableOpacity>
                  )}

                  <Text style={styles.inputLabel}>Rejection Notes (Only if rejecting)</Text>
                  <TextInput
                ...
                <View style={styles.actionRow}>
                  ...
                  <TouchableOpacity 
                    ...
                    onPress={() => handleReviewAction('reimbursed')}
                    disabled={isProcessing}
                  >
                    <Text style={styles.actionBtnText}>Reimburse & Pay</Text>
                  </TouchableOpacity>
                </View>
              </>
            )}
