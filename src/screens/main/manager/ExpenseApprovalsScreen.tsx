...
export default function ExpenseApprovalsScreen() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAllExpenses(
      ['pending_manager', 'pending_finance', 'reimbursed', 'rejected'],
      (data) => {
        setExpenses(data);
        setIsLoading(false);
      }
  ...
  const filteredExpenses = useMemo(() => {
    ...
    if (filter === 'approved') return expenses.filter(e => e.status === 'pending_finance' || e.status === 'reimbursed');
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <View style={styles.filterRow}>
        {(['pending_manager', 'approved', 'rejected', 'all'] as const).map(f => (
          <TouchableOpacity 
            key={f}
          ...
          >
            <Text style={[styles.filterTxt, filter === f && styles.filterTxtActive]}>
              {f === 'pending_manager' ? 'Pending' : f.charAt(0).toUpperCase() + f.slice(1)}
            </Text>
            {f === 'pending_manager' && expenses.filter(e => e.status === 'pending_manager').length > 0 && (
